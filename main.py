...
def load_data_from_gsheets(url):
    try:
        ...
        colunas_lancamentos = ['Data', 'Obra', 'Funcionário', 'Disciplina', 'Serviço', 'Quantidade', 'Unidade', 'Valor Unitário', 'Valor Parcial']
...
def login_page(obras_df):
    ...
    if not codigos_obras:
        ...
        st.code("""
        # Adicione no seu arquivo secrets.toml:
        ["códigos_obras"]
        "NOME DA OBRA 1" = "codigo123"
        "NOME DA OBRA 2" = "senha456"
        """)
        return
    ...
    if st.session_state.page == "Lançamento Folha 📝":
                ...
                if submitted:
                    if not all([obra_selecionada, funcionario_selecionado]):
                        ...
                        def prepare_row_for_gsheet(data_dict):
                            ...
                            if isinstance(row_values[0], datetime):
                                row_values[0] = row_values[0].strftime("%Y-%m-%d %H:%M:%S")
        ...
        if not st.session_state.lancamentos:
            ...
            if df_filtrado.empty:
                ...
                colunas_visiveis = ['Remover', 'Data', 'Obra', 'Funcionário', 'Serviço', 'Quantidade', 'Valor Parcial', 'id_lancamento']
        ...
        if lancamentos_da_obra.empty:
            ...
            if df_filtrado_dash.empty:
                st.warning("Nenhum lançamento encontrado para o período selecionado nesta obra.")
            else:
                st.markdown("---")
                kpi1, kpi2 = st.columns(2)
                total_produzido = df_filtrado_dash['Valor Parcial'].sum()
                kpi1.metric("Produção Total no Período", format_currency(total_produzido))
                
                top_funcionario = df_filtrado_dash.groupby('Funcionário')['Valor Parcial'].sum().idxmax()
                kpi2.metric("Funcionário Destaque na Obra", top_funcionario)
