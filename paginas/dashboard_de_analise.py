...
def render_page():
    ...
    if not df_filtrado_lanc.empty:
        ...
        if st.session_state['role'] == 'admin':
            ...
            lanc_sem_grat_detalhe = df_filtrado_lanc[df_filtrado_lanc['Disciplina'] != 'GRATIFICAÇÃO'] if not df_filtrado_lanc.empty else pd.DataFrame()
            if not lanc_sem_grat_detalhe.empty:
                with col_serv:
                    serv_custo = lanc_sem_grat_detalhe.groupby('Serviço')['Valor Parcial'].sum().nlargest(10).reset_index().sort_values('Valor Parcial', ascending=True)
                ...
                with col_disc:
                    disc_custo = lanc_sem_grat_detalhe.groupby('Disciplina')['Valor Parcial'].sum().nlargest(10).reset_index().sort_values('Valor Parcial', ascending=True)
    ...
    if st.session_state['role'] == 'admin':
        if not folhas_df.empty:
            ...
            with col_prazo2:
             ...
             st.info(f"Nenhuma folha encontrada para o mês {mes_selecionado}
